...
export async function GET(
  ...
  const isLiked = !!(await prisma.blog.findFirst({
    where: {
      id,
      likedBy: {
        some: {
  ...
  const isDisliked = !!(await prisma.blog.findFirst({
    where: {
      id,
      dislikedBy: {
        some: {
...
export async function DELETE(
  ...
  //remove likes, comments before deleting
